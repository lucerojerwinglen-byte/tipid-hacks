...
correction: KFC's and Shakey's real menus are dense enough that the LLM-extraction step (§4)
reliably stalls on Groq's free-tier rate limit for 20-50+ minutes per chunk, both locally and in
a real GitHub Actions run (2026-08-12/13). Both moved to hand-maintained data
(`src/data/{kfc,shakeys}.ts`), joining Chowking — three of six chains are hand-maintained, three
(Jollibee, McDonald's, Mang Inasal) run through the automated pipeline.
